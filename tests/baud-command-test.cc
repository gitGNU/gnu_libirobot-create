...
{
  robot.sendBaudCommand (iRobot::Create::BAUD_300);
  robot.sendBaudCommand (iRobot::Create::BAUD_19200);
  robot.sendBaudCommand (iRobot::Create::BAUD_115200);

  try
    {
      // Send an invalid baud value.
      robot.sendBaudCommand (static_cast<iRobot::Create::Baud> (255));
    }
  catch (iRobot::InvalidArgument&)
    {
      return TEST_SUCCEED;
    }
  catch (...)
